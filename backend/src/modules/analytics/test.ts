...
async function runTests() {
  ...
  const admin = await UserModel.findOne({ role: SystemRoles.MANAGEMENT_ADMIN });
  const recruiter = await UserModel.findOne({ role: SystemRoles.HR_RECRUITER });
  ...
  const recruiterJobs = await JobModel.find({ createdBy: recruiter._id as any, isDeleted: false }, '_id');
  const recruiterJobIds = recruiterJobs.map((j) => j._id);
  const recruiterAppFilter = { jobId: { $in: recruiterJobIds } };
  const recruiterJobFilter = { createdBy: recruiter._id as any };

  console.log('--------------------------------------------------');
  console.log('1. Testing Recruitment Overview...');
  const adminOverview = await analyticsRepository.getRecruitmentOverview(
    adminAppFilter,
    adminJobFilter,
  ...
  const recOverview = await analyticsRepository.getRecruitmentOverview(
    recruiterAppFilter,
    recruiterJobFilter,
    {}
  );
  ...
  const adminFunnel = await analyticsRepository.getRecruitmentFunnel(adminAppFilter);
  ...
  const screening = await analyticsRepository.getAIScreeningStats(adminAppFilter);
  ...
  const interviews = await analyticsRepository.getAIInterviewStats(adminAppFilter);
  ...
  const recruitersList = await analyticsRepository.getRecruiterPerformance(adminAppFilter);
  ...
  const departmentsList = await analyticsRepository.getDepartmentHiringStats(adminAppFilter);
  console.log('Departments Stats:', departmentsList);

  console.log('--------------------------------------------------');
  console.log('7. Testing Job Performance (Pagination)...');
  const jobPerformance = await analyticsRepository.getJobPerformance(adminAppFilter, { page: 1, limit: 3 });
  ...
  const skills = await analyticsRepository.getSkillsIntelligence(adminJobFilter, adminCandidateFilter);
  ...
  const activity = await analyticsRepository.getRecruitmentActivityStats(adminAppFilter);
