...
export default function AttendanceAnalyticsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const s = await attendanceService.getSummary(employeeId, month, year);
      setSummary(s);
  ...
  const bars = summary ? [
    { label: 'Present',  val: summary.presentDays,  total: summary.totalWorkingDays, color: 'bg-emerald-500', text: 'text-emerald-600' },
    { label: 'Late',     val: summary.lateDays,      total: summary.totalWorkingDays, color: 'bg-amber-400',  text: 'text-amber-600'   },
    { label: 'Absent',   val: summary.absentDays,    total: summary.totalWorkingDays, color: 'bg-red-500',    text: 'text-red-600'     },
    { label: 'On Leave', val: summary.leaveDays,     total: summary.totalWorkingDays, color: 'bg-blue-500',   text: 'text-blue-600'    },
  ] : [];
      ...
      <Card className="border-zinc-200/60 dark:border-zinc-800">
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="flex flex-col md:flex-row items-center gap-8">
              ...
              <div className="relative shrink-0">
                <svg className="w-40 h-40 -rotate-90" viewBox="0 0 120 120">
                  <circle cx="60" cy="60" r="50" fill="none" stroke="currentColor" strokeWidth="12" className="text-zinc-100 dark:text-zinc-800" />
                  <circle
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-3xl font-extrabold text-zinc-900 dark:text-white">{pct.toFixed(0)}%</span>
            ...
            <div className="flex items-center justify-center py-14 text-zinc-400 text-sm">
              No summary data. Select a period and click Refresh.
            </div>
          )}
        </CardContent>
      </Card>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {statCards.map(({ label, val, sub, icon: Ic, color }) => (
            ...
            >
              ...
              <p className="text-2xl font-extrabold text-zinc-900 dark:text-white">{val}</p>
