...
const interview_repository_1 = __importDefault(require("./interview.repository"));
...
const InterviewQuestion_1 = __importDefault(require("../../../models/InterviewQuestion"));
const InterviewResponse_1 = __importDefault(require("../../../models/InterviewResponse"));
const InterviewReport_1 = __importDefault(require("../../../models/InterviewReport"));
const InterviewEvaluation_1 = __importDefault(require("../../../models/InterviewEvaluation"));
...
function normalizeCategory(category) {
    ...
    if (cat.includes('technical') || cat.includes('tech'))
        return 'Technical';
    if (cat.includes('behavioral') || cat.includes('behav'))
        return 'Behavioral';
    if (cat.includes('situational') || cat.includes('situat'))
        return 'Situational';
    if (cat.includes('problem solving') || cat.includes('problem'))
        return 'Problem Solving';
    if (cat.includes('project-based') || cat.includes('project'))
        return 'Project-Based';
    if (cat.includes('resume-based') || cat.includes('resume'))
        return 'Resume-Based';
...
function normalizeSourceType(sourceType) {
    ...
    if (src.includes('resume'))
        return 'Resume';
    if (src.includes('jobdescription') || src.includes('job') || src.includes('description'))
        return 'JobDescription';
    if (src.includes('followup') || src.includes('follow'))
        return 'FollowUp';
    if (src.includes('behavioral') || src.includes('behav'))
        return 'Behavioral';
    if (src.includes('technical') || src.includes('tech'))
        return 'Technical';
...
class InterviewService {
    ...
    async createTemplate(data, userId) {
        return await interview_repository_1.default.createTemplate({
            ...data,
            createdBy: userId,
            updatedBy: userId
        });
    ...
    async getTemplates(filter = {}) {
        return await interview_repository_1.default.findTemplates(filter);
    }
    async getTemplateById(id) {
        const template = await interview_repository_1.default.findTemplateById(id);
        if (!template)
            throw new Error('Interview template not found');
        return template;
    }
    async updateTemplate(id, data, userId) {
        return await interview_repository_1.default.updateTemplate(id, {
            ...data,
            updatedBy: userId
        });
    }
    async deleteTemplate(id) {
        return await interview_repository_1.default.deleteTemplate(id);
    }
    // --- ASSIGNMENTS ---
    async getAssignments(filter = {}) {
        return await interview_repository_1.default.findAssignments(filter);
    }
    async getAssignmentById(id) {
        const assignment = await interview_repository_1.default.findAssignmentById(id);
        if (!assignment)
            throw new Error('Interview assignment not found');
        return assignment;
    }
    // --- HELPER TO GENERATE AND SAVE QUESTIONS ---
    async generateAndSaveInitialQuestions(session, job, template, assignment) {
        ...
        return await InterviewQuestion_1.default.insertMany(questionsToSave);
    }
    // --- CANDIDATE SESSION MANAGEMENT ---
    async startSession(assignmentId, candidateId) {
        const assignment = await InterviewAssignment_1.default.findById(assignmentId)
        ...
        let template = assignment.interviewTemplateId;
        ...
        if (!template && job && job.interviewTemplateId) {
            ...
            const InterviewTemplate = mongoose_1.default.model('InterviewTemplate');
            template = await InterviewTemplate.findById(job.interviewTemplateId);
        }
        // Check if there is an in-progress session to resume
        let session = await InterviewSession_1.default.findOne({ assignmentId, status: { $in: ['Started', 'In Progress'] } });
        if (session) {
            // Return existing session and its current question
            let currentQuestion = await InterviewQuestion_1.default.findOne({
                sessionId: session._id,
        ...
        // Create session
        session = await InterviewSession_1.default.create({
            assignmentId,
            candidateId,
    ...
    async submitAnswer(sessionId, questionId, file) {
        const session = await InterviewSession_1.default.findById(sessionId);
        const question = await InterviewQuestion_1.default.findById(questionId);
        if (!session || !question) {
            throw new Error('Interview session or question not found');
        }
        // Ensure upload directory exists
        const uploadDir = path_1.default.join(process.cwd(), 'uploads', 'interviews');
        if (!fs_1.default.existsSync(uploadDir)) {
            fs_1.default.mkdirSync(uploadDir, { recursive: true });
        }
        // Move file to unique location under uploads
        const tempPath = file.path;
        const finalFilename = `${sessionId}_${questionId}_${Date.now()}${path_1.default.extname(file.originalname)}`;
        ...
        const response = await InterviewResponse_1.default.create({
            sessionId,
            questionId,
            audioFileUrl,
            transcript: '',
        ...
        const redisOnline = await (0, queue_config_1.isRedisOnline)();
        if (redisOnline) {
            console.log(`[Interview Service] Redis is online. Enqueuing process-answer job in BullMQ...`);
            await queue_config_1.interviewQueue.add('process-answer', {
                sessionId,
        ...
        const updatedSession = await InterviewSession_1.default.findById(sessionId);
        let nextQuestion = null;
        if (updatedSession) {
            nextQuestion = await InterviewQuestion_1.default.findOne({
    ...
    // --- INTEGRITY TRACKING ---
    async updateIntegrity(sessionId, eventType) {
        const session = await InterviewSession_1.default.findById(sessionId);
        if (!session)
    ...
    async getReport(sessionId) {
        let report = await InterviewReport_1.default.findOne({ sessionId })
            .populate({
            path: 'sessionId',
        ...
        if (!report) {
            ...
            const activeSession = await InterviewSession_1.default.findOne({ assignmentId: sessionId });
            if (activeSession) {
                sessionId = activeSession._id.toString();
                report = await InterviewReport_1.default.findOne({ sessionId: activeSession._id })
                    .populate({
                    path: 'sessionId',
                    populate: { path: 'assignmentId' }
                })
        ...
        if (!report)
            throw new Error('Evaluation report not ready or compile failed.');
        // Fetch full timeline details (questions + transcripts + individual evaluation scores)
        const questions = await InterviewQuestion_1.default.find({ sessionId }).sort({ order: 1 }).lean();
        const responses = await InterviewResponse_1.default.find({ sessionId }).lean();
        const evaluations = await InterviewEvaluation_1.default.find({ sessionId }).lean();
    ...
    async getAnalytics(filter = {}) {
        return await interview_repository_1.default.getAggregatedAnalytics(filter);
    }
    async processAnswer(sessionId, responseId, audioFilePath, questionText) {
        ...
        const response = await InterviewResponse_1.default.findById(responseId);
        const session = await InterviewSession_1.default.findById(sessionId);
        if (!response || !session) {
        ...
        await response.save();
        const assignment = await InterviewAssignment_1.default.findById(session.assignmentId);
        if (!assignment) {
        ...
        if (fs_1.default.existsSync(audioFilePath)) {
            ...
            formData.append('file', fileBlob, path_1.default.basename(audioFilePath));
            try {
        ...
        await InterviewEvaluation_1.default.create({
            sessionId: session._id,
            responseId: response._id,
        ...
        if (currentOrder < maxQuestions) {
            ...
            await InterviewQuestion_1.default.create({
                sessionId: session._id,
            ...
            if (redisOnline) {
                ...
                await queue_config_1.interviewQueue.add('generate-report', { sessionId: session._id });
            }
            else {
    ...
    async compileReport(sessionId) {
        ...
        const questions = await InterviewQuestion_1.default.find({ sessionId }).sort({ order: 1 });
        const responses = await InterviewResponse_1.default.find({ sessionId });
        const evaluations = await InterviewEvaluation_1.default.find({ sessionId });
        ...
        await InterviewReport_1.default.findOneAndUpdate({ sessionId: session._id }, {
        ...
        await ranking_service_1.RankingService.updateRankingsForJob(assignment.jobId.toString());
