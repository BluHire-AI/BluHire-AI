...
import { chatQuerySchema, actionConfirmSchema } from './copilot.validator';
...
export class CopilotController {
   ...
   */
  async chatStream(req: any, res: Response): Promise<void> {
    ...
    const writeSSE = (type: string, content?: any, conversationId?: string, followUps?: string[]) => {
  ...
  async getConversations(req: any, res: Response): Promise<void> {
    try {
      const userId = req.user._id;
      const conversations = await CopilotConversation.find({ userId, isActive: true })
        .sort({ updatedAt: -1 })
        .limit(50);
      res.json(createSuccessResponse(conversations, 'Chat sessions retrieved successfully'));
    } catch (error: any) {
      res.status(500).json(createErrorResponse(error.message || 'Failed to list chat sessions'));
    }
  }
  ...
  async getMessages(req: any, res: Response): Promise<void> {
    try {
      ...
      // Verify ownership
      const conversation = await CopilotConversation.findOne({ _id: conversationId, userId, isActive: true });
      if (!conversation) {
        res.status(404).json(createErrorResponse('Conversation not found'));
        return;
      }

      const messages = await CopilotMessage.find({ conversationId }).sort({ timestamp: 1 });
      res.json(createSuccessResponse(messages, 'Message logs retrieved successfully'));
    } catch (error: any) {
      res.status(500).json(createErrorResponse(error.message || 'Failed to retrieve message logs'));
    }
  }
   ...
   */
  async deleteConversation(req: any, res: Response): Promise<void> {
    try {
      const { conversationId } = req.params;
      const userId = req.user._id;
      ...
      res.json(createSuccessResponse(null, 'Chat conversation deleted successfully'));
    } catch (error: any) {
      res.status(500).json(createErrorResponse(error.message || 'Failed to delete chat session'));
    }
  }
   ...
   */
  async getReports(req: any, res: Response): Promise<void> {
    try {
      const userId = req.user._id;
      const reports = await CopilotReport.find({ generatedBy: userId })
        .sort({ createdAt: -1 })
        .select('_id reportName reportType createdAt');
      res.json(createSuccessResponse(reports, 'Saved reports listed successfully'));
    } catch (error: any) {
      res.status(500).json(createErrorResponse(error.message || 'Failed to retrieve saved reports list'));
    }
  }
   ...
   */
  async getReportDetails(req: any, res: Response): Promise<void> {
    try {
      const { reportId } = req.params;
      const userId = req.user._id;

      const report = await CopilotReport.findOne({ _id: reportId, generatedBy: userId });
      if (!report) {
        res.status(404).json(createErrorResponse('Report record not found'));
        return;
      }

      res.json(createSuccessResponse(report, 'Report details retrieved successfully'));
    } catch (error: any) {
      res.status(500).json(createErrorResponse(error.message || 'Failed to retrieve report details'));
    }
  }
  ...
  async exportReport(req: any, res: Response): Promise<void> {
    try {
      ...
      const report = await CopilotReport.findOne({ _id: reportId, generatedBy: userId });
      if (!report) {
        res.status(404).json(createErrorResponse('Report not found'));
        return;
      }
      ...
      res.status(400).json(createErrorResponse('Unsupported export format'));
    } catch (error: any) {
