  ...
  async generateSummaryForEmployee(employeeId: string, month: number, year: number): Promise<IAttendanceSummary> {
    ...
    for (const att of attendances) {
      if (att.attendanceStatus === AttendanceStatus.PRESENT || 
        ...
        lateDays++;
        presentDays++; 
      } else if (att.attendanceStatus === AttendanceStatus.HALF_DAY) {
        halfDays++;
        presentDays += 0.5;
      } else if (att.attendanceStatus === AttendanceStatus.HOLIDAY) {
        holidayDays++;
        presentDays++;
    ...
    const tHours = Number(totalHours.toFixed(2));
    const oHours = Number(overtimeHours.toFixed(2));
    ...
    return await attendanceSummaryRepository.upsert(
      ...
      year,
      {
        ...
        totalWorkingHours: tHours,
        totalOvertimeHours: oHours,
