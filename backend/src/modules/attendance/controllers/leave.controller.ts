...
export class LeaveController {
  ...
  async getLeaveBalance(req: any, res: Response): Promise<void> {
    try {
      ...
      const balance = await leaveService.getLeaveBalance(employeeId, Number(year));
      res.json(createSuccessResponse(balance, 'Leave balance retrieved successfully'));
    } catch (error: any) {
      res.status(400).json(createErrorResponse(error.message || 'Failed to retrieve leave balance', undefined, 400));
    }
  }
}
